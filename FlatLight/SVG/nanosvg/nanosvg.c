...
static struct SVGHelper* svgCreateParser()
{
	struct SVGHelper* p;
	p = (struct SVGHelper*)malloc(sizeof(struct SVGHelper));
	if (!p)
		return NULL;
	memset(p, 0, sizeof(struct SVGHelper));
...
static void svgDeleteParser(struct SVGHelper* p)
{
	struct SVGPath* path;
	struct SVGPath* next;
	path = p->plist;
...
static void svgResetPath(struct SVGHelper* p)
{
	p->nbuf = 0;
}

static void svgPathPoint(struct SVGHelper* p, float x, float y)
{
	int cap;
	float* buf;
	if (p->nbuf+1 > p->cbuf)
...
static struct SVGAttrib* svgGetAttr(struct SVGHelper* p)
{
	return &p->attr[p->attrHead];
}

static void svgPushAttr(struct SVGHelper* p)
{
...
static void svgPopAttr(struct SVGHelper* p)
{
	if (p->attrHead > 0)
		p->attrHead--;
}

static void svgCreatePath(struct SVGHelper* p, char closed)
{
	float* t;
	float* pt;
...
static int svgParseMatrix(struct SVGHelper* p, const char* str)
{
	float t[6];
	int na = 0;
...
static int svgParseTranslate(struct SVGHelper* p, const char* str)
{
	float args[2];
	float t[6];
	int na = 0;
...
static int svgParseScale(struct SVGHelper* p, const char* str)
{
	float args[2];
	int na = 0;
	float t[6];
...
static void svgParseTransform(struct SVGHelper* p, const char* str)
{
	while (*str)
	{
...
static void svgParseStyle(struct SVGHelper* p, const char* str);

static int svgParseAttr(struct SVGHelper* p, const char* name, const char* value)
{
...
static int svgParseNameValue(struct SVGHelper* p, const char* start, const char* end)
{
	const char* str;
	const char* val;
	char name[512];
...
static void svgParseAttribs(struct SVGHelper* p, const char** attr)
{
	int i;
	for (i = 0; attr[i]; i += 2)
	{
...
static void cubicBezRec(struct SVGHelper* p,
...
static void cubicBez(struct SVGHelper* p,
...
static void quadBezRec(struct SVGHelper* p,
...
static void quadBez(struct SVGHelper* p,
...
static void pathLineTo(struct SVGHelper* p, float* cpx, float* cpy, float* args, int rel)
{
	if (rel)
	{
		*cpx += args[0];
...
static void pathHLineTo(struct SVGHelper* p, float* cpx, float* cpy, float* args, int rel)
{
	if (rel)
		*cpx += args[0];
	else
...
static void pathVLineTo(struct SVGHelper* p, float* cpx, float* cpy, float* args, int rel)
{
	if (rel)
		*cpy += args[0];
	else
...
static void pathCubicBezTo(struct SVGHelper* p, float* cpx, float* cpy,
...
static void pathCubicBezShortTo(struct SVGHelper* p, float* cpx, float* cpy,
								float* cpx2, float* cpy2, float* args, int rel)
{
	float x1, y1, x2, y2, cx1, cy1, cx2, cy2;
...
static void pathQuadBezTo(struct SVGHelper* p, float* cpx, float* cpy,
						  float* cpx2, float* cpy2, float* args, int rel)
{
...
static void pathQuadBezShortTo(struct SVGHelper* p, float* cpx, float* cpy,
							   float* cpx2, float* cpy2, float* args, int rel)
{
	float x1, y1, x2, y2, cx, cy;
...
static void svgParsePath(struct SVGHelper* p, const char** attr)
{
	const char* s;
	char cmd;
	float args[10];
...
static void svgParseRect(struct SVGHelper* p, const char** attr)
{
	float x = 0.0f;
	float y = 0.0f;
	float w = 0.0f;
...
static void svgParseCircle(struct SVGHelper* p, const char** attr)
{
	float cx = 0.0f;
	float cy = 0.0f;
	float r = 0.0f;
...
static void svgParseLine(struct SVGHelper* p, const char** attr)
{
	float x1 = 0.0;
	float y1 = 0.0;
	float x2 = 0.0;
...
static void svgParsePoly(struct SVGHelper* p, const char** attr, int closeFlag)
{
	int i;
	const char* s;
	float args[2];
...
{
	struct SVGHelper* p = (struct SVGHelper*)ud;
